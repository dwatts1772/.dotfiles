...
SPACESHIP_ENV_PREFIX="${SPACESHIP_ENV_PREFIX:="$SPACESHIP_PROMPT_DEFAULT_PREFIX"}"
SPACESHIP_ENV_SUFFIX="${SPACESHIP_ENV_SUFFIX:="$SPACESHIP_PROMPT_DEFAULT_SUFFIX"}"
SPACESHIP_ENV_COLOR="${SPACESHIP_ENV_COLOR:="orange"}"
...
# .ENV file
# Show current db host from .env
spaceship_env(){
  [[ $SPACESHIP_ENV_SHOW == false ]] && return
  ...
  _prompt_section \
    "$SPACESHIP_ENV_COLOR" \
    "$SPACESHIP_ENV_PREFIX" \
    "$ENV_DB:t" \
    "$SPACESHIP_ENV_SUFFIX"
}
